...
def copy_board(full_board,level):
    ...
    remove={'Easy':30,'Medium':40,'Hard':50}
    count=remove.get(level,40)
    while count>0:
...
f4=('Helvetica',18)
root2.configure(bg=clr4)
l1=Label(text='Sudoku: The 9x9 Challenge',font=f1,bg=clr4,fg=clr1)
l1.pack(pady=10)
con=Frame(bg=clr4)
con.pack(padx=10,pady=10)
grid_f=Frame(con,bg=clr1,padx=4,pady=4)
grid_f.grid(row=0,column=0,padx=(10,90),pady=0)
grid_btn=[]
for i in range(9):
    row=[]
    for j in range(9):
        x=(4,1)if j%3==0 else (1,1)
        if j==8:
            x=(1,4)
        y=(4,1)if i%3==0 else (1,1)
        if i==8:
            y=(1,4)
        cell_f=Frame(grid_f, bg=clr1)
        cell_f.grid(row=i,column=j,padx=x,pady=y)
        btn=Button(cell_f,text='',font=f4,width=3,height=1,relief='flat',bg=clr5,fg=clr1,bd=1)
        btn.pack()
        row.append(btn)
    grid_btn.append(row)
...
b5=Button(side_f,text='Exit',font=f3,width=10,height=1,bg=clr3,activebackground=clr1,fg=clr5,command=root2.destroy)
b6=Button(side_f,text='4x4 Quest',font=f3,width=20,height=1,bg=clr3,activebackground=clr1,fg=clr5,command=open4)
...
b6.grid(row=4,column=0,columnspan=2,pady=30,padx=10)
num_f=Frame(bg=clr4)
num_f.pack(pady=1)
...
for n in range(1,10):
    ...
    btn.pack(side='left',padx=((40,5) if n==1 else 5))
generate()
mainloop()